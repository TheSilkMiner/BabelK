...
internal class ScriptFinalizableGrammar(private val grammarName: String, private val grammarPack: ScriptCollectionGrammarPack) : ThisGrammar {
    private class FinalizedCollection(private val collection: NamedObjectCollection<NamedRule>) : MutableNamedObjectCollection<NamedRule, Rule> {
        override fun byName(name: String): NamedObjectProvider<NamedRule> = this.collection.byName(name)
        override fun register(name: String, creator: () -> Rule): NamedObjectProvider<NamedRule> = error("Unavailable -- this should never be invoked!")
    ...
    override val rules: MutableNamedObjectCollection<NamedRule, Rule> get() = this.rulesCollection.finalizeIfNeeded()

    private fun MutableNamedObjectCollection<NamedRule, Rule>.finalizeIfNeeded(): MutableNamedObjectCollection<NamedRule, Rule> {
