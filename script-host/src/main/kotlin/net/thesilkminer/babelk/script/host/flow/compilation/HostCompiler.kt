...
private fun Script.compile(): LoadableScript {
    ...
    val script = this.compileWithResult(builtinName).reportDiagnostics(log).rethrowOnError(this)
...
private fun ResultWithDiagnostics<CompiledScript>.rethrowOnError(script: Script): CompiledScript {
    return this.rethrowOnError(log) { "Compilation errors have occurred while attempting to compile script ${script.name}: please check the compiler output" }
}
